...
docker service create --name pg-test-secret --network my-lab \
            --secret pg-password --secret pg-username --secret pg-bdd \ # sends the secrets variables to the container
            -e POSTGRES_PASSWORD_FILE="/run/secrets/pg-password" \ # access to the pg-password secret variable
            -e POSTGRES_USER_FILE="/run/secrets/pg-username" \
            -e POSTGRES_DB_FILE="/run/secrets/pg-bdd" postgres

docker service create -p 99:80 --name=pgadmin --network my-lab \ # we raise this service just for connect to the database via web
